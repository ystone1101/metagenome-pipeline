...
#--- KneadData 실행 함수 (내장 FastQC 사용) ---
run_kneaddata() {
    local sample_name=$1; local r1_uncompressed=$2; local r2_uncompressed=$3
    local ref_db=$4; local work_dir=$5; local clean_dir=$6; local kneaddata_log_dir=$7
    local fastqc_pre_dir=$8; local fastqc_post_dir=$9; 
    local trimmomatic_options_value="${10}"
    ...
    local kneaddata_args=(
        ...
        --max-memory "$KNEADDATA_MAX_MEMORY" --output "$kneaddata_out_dir"
        --trimmomatic-options "$trimmomatic_options_value" --remove-intermediate-output
        -t "$KNEADDATA_THREADS" -p "$KNEADDATA_PROCESSES"
        --run-fastqc-start --run-fastqc-end
    )
    
    conda run -n "$KNEADDATA_ENV" kneaddata "${kneaddata_args[@]}" > "$console_log" 2>&1
    ...
    local r1_final_path="${work_dir}/$(basename "$r1_gz")"; local r2_final_path="${work_dir}/$(basename "$r2_gz")"
...
#--- Kraken2 실행 함수 (자체 Checkpoint 및 효율적 통계 계산) ---
run_kraken2() {
    local sample_name=$1; local r1_clean_gz=$2; local r2_clean_gz=$3; local kraken_db=$4
    local kraken_out_dir=$5; local summary_tsv_file=$6; local mpa_out_dir=$7
    ...
    if [[ -f "$k2_output" && -f "$k2_report" ]]; then
        log_info "${sample_name}: 기존 Kraken2 결과 파일을 발견했습니다. 통계 계산만 다시 수행합니다."
    else
        log_info "${sample_name}: Kraken2 실행 중 (환경: ${KRAKEN_ENV})..."
        conda run -n "$KRAKEN_ENV" kraken2 \
            --db "$kraken_db" --threads "$KRAKEN2_THREADS" --report "$k2_report" --paired \
            --report-minimizer-data --minimum-hit-groups 3 \
            "$r1_clean_gz" "$r2_clean_gz" > "$k2_output"
        log_info "${sample_name}: Kraken2 실행 완료."
    fi
    
    log_info "${sample_name}: Kraken2 분류 통계 계산 중 (.k2report 파일 사용)..."
    local summary_stats=($(head -n 2 "$k2_report" | awk '{print $2}'))
    local UNCLASSIFIED=${summary_stats[0]:-0}
    local CLASSIFIED=${summary_stats[1]:-0}
    ...
    log_info "${sample_name}: kreport2mpa.py 변환 중..."
    cut -f1-3,6-8 "$k2_report" > "$k2_report_6col"
    conda run -n "$KRAKEN_ENV" kreport2mpa.py -r "$k2_report_6col" -o "$mpa_reads" --display-header --no-intermediate-ranks
    conda run -n "$KRAKEN_ENV" kreport2mpa.py --percentages -r "$k2_report_6col" -o "$mpa_percent" --display-header --no-intermediate-ranks
    rm "$k2_report_6col"
}
