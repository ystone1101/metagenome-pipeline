...
for R1 in "$RAW_DIR"/*_1.fastq.gz; do
    ...
    run_kneaddata_flag=true

    if [[ -f "$r1_for_kraken2" && -f "$r2_for_kraken2" ]]; then
        log_info "${SAMPLE}: WORK_DIR에서 이미 처리된 KneadData 파일을 발견했습니다. KneadData 단계를 건너<binary data, 2 bytes><binary data, 2 bytes><binary data, 2 bytes>니다."
        run_kneaddata_flag=false
    else
        ...
        if ! cleaned_files_str=$(run_kneaddata "$SAMPLE" "$r1_uncompressed" "$r2_uncompressed" \
            ...
            log_warn "KneadData 처리 중 오류가 발생하여 ${SAMPLE} 분석을 건너<binary data, 2 bytes><binary data, 2 bytes><binary data, 2 bytes>니다. 다음 샘플로 진행합니다."
        ...
        if [[ -z "${cleaned_files[0]}" ]]; then
            log_warn "${SAMPLE}: 후속 분석을 위한 파일이 없어 건너<binary data, 2 bytes><binary data, 2 bytes><binary data, 2 bytes>니다."
            # 이 경우에는 r1_uncompressed가 이미 위에서 삭제되었으므로 추가 삭제 불필요
            continue
        fi
    ...
    log_info "${SAMPLE}: WORK_DIR의 임시 파일들을 삭제합니다."
    # Checkpoint를 통해 KneadData를 건너뛴 경우에는 uncompressed 파일이 없으므로, 삭제 시도하지 않음
    if [[ "$run_kneaddata_flag" == true ]]; then
        # 신규 분석을 했을 경우에만 압축 해제된 원본 파일이 위에서 생성되고 삭제되었으므로, 여기서는 할 일 없음.
        # 이 if문은 안전을 위해 남겨두거나, 혹은 더 단순하게 아래 한 줄만 남겨도 됨.
        : # 아무것도 하지 않음
    fi
    rm "$r1_for_kraken2" "$r2_for_kraken2"
